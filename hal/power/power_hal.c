...
#define LOG_TAG "Exynos5422PowerHAL"
/* #define LOG_NDEBUG 0 */
#include <utils/Log.h>
...
struct exynos5422_power_module {
...
static int boostpulse_open(struct exynos5422_power_module *exynos5422_pwr)
{
    char errno_str[64];

    if (exynos5422_pwr->boostpulse_fd < 0) {
        exynos5422_pwr->boostpulse_fd = open(BOOSTPULSE_PATH, O_WRONLY);
        if (exynos5422_pwr->boostpulse_fd < 0) {
            if (!exynos5422_pwr->boostpulse_warned) {
                strerror_r(errno, errno_str, sizeof(errno_str));
                ALOGE("Error opening %s: %s\n", BOOSTPULSE_PATH, errno_str);
                exynos5422_pwr->boostpulse_warned = 1;
            }
        }
    }

    return exynos5422_pwr->boostpulse_fd;
}
...
static void init_touchscreen_power_path(struct exynos5422_power_module *exynos5422_pwr)
{
    exynos5422_pwr->touchscreen_power_path = TOUCHSCREEN_PATH;
}

static void init_touchkey_power_path(struct exynos5422_power_module *exynos5422_pwr)
{
    exynos5422_pwr->touchkey_power_path = TOUCHKEY_PATH;
}

static void init_gpio_keys_power_path(struct exynos5422_power_module *exynos5422_pwr)
{
    ...
    while ((de = readdir(d)) != NULL) {
        if (strncmp("input", de->d_name, 5) == 0) {
            ...
            exynos5422_pwr->gpio_keys_power_path = path;
 ...
 */
static void exynos5422_power_init(struct power_module *module)
{
    struct exynos5422_power_module *exynos5422_pwr = (struct exynos5422_power_module *) module;
    struct stat sb;
    int rc;
...
out:
    init_gpio_keys_power_path(exynos5422_pwr);
    init_touchscreen_power_path(exynos5422_pwr);
    init_touchkey_power_path(exynos5422_pwr);
}
 ...
 * on the screen.
 */
static void exynos5422_power_set_interactive(struct power_module *module, int on)
{
    struct exynos5422_power_module *exynos5422_pwr = (struct exynos5422_power_module *) module;
    struct stat sb;
    char buf[80];
    ...
    sysfs_write(exynos5422_pwr->touchscreen_power_path, on ? "1" : "0");
    sysfs_write(exynos5422_pwr->touchkey_power_path, on ? "1" : "0");
    sysfs_write(exynos5422_pwr->gpio_keys_power_path, on ? "1" : "0");
}
 ...
 */
static void exynos5422_power_hint(struct power_module *module,
                                  power_hint_t hint,
                                  void *data)
{
    struct exynos5422_power_module *exynos5422_pwr =
        (struct exynos5422_power_module *)module;
    ...
    switch (hint) {
        case POWER_HINT_INTERACTION: {
            ...
            pthread_mutex_lock(&exynos5422_pwr->lock);
            fd = boostpulse_open(exynos5422_pwr);
            if (fd >= 0) {
                len = write(exynos5422_pwr->boostpulse_fd, "1", 1);
                if (len < 0) {
                    strerror_r(errno, errno_str, sizeof(errno_str));
                    ALOGE("Error writing to %s: %s\n", BOOSTPULSE_PATH, errno_str);
                } else {
            ...
            pthread_mutex_unlock(&exynos5422_pwr->lock);
...
static int exynos5422_get_feature(struct power_module *module __unused,
                                  feature_t feature)
{
...
struct exynos5422_power_module HAL_MODULE_INFO_SYM = {
    .base = {
        .common = {
        ...
        .init = exynos5422_power_init,
        .setInteractive = exynos5422_power_set_interactive,
        .powerHint = exynos5422_power_hint,
        .getFeature = exynos5422_get_feature
    },
